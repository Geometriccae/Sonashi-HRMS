...
import EditEventModal from "./EditAssignTaskModal";
import DeleteModal from "../delete-modal/DeleteModal";
import {
  getEventsByEmployeeId,
  deleteEvent,
} from "../../services/AssignEventService";
...
const MeetingsTable = ({ employeeId, key: calendarKey }) => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        setIsLoading(true);
        const fetchedEvents = await getEventsByEmployeeId(employeeId);
    ...
    fetchEvents();
  }, [employeeId, calendarKey]); // Add calendarKey to dependencies for refresh

  // Sample events for fallback/demonstration
  const sampleEvents = [
    {
      id: 1,
      title: "General with dev team on Issue #47",
  ...
  const confirmDeleteEvent = async () => {
    ...
    try {
      await deleteEvent(employeeId, eventToDelete.id);
          ...
          >
            <div className={styles["modal-header"]}>
              <div className={styles["meeting-chip"]}>
                ...
                <span>{format(selectedMeeting.start, "h:mm a")}</span>
              </div>
